...
#   (1...words.length).map do |i|
#     words[i].capitalize! unless minor_words.include? words[i]
#   end
#   words.join(" ")
# end

# def title_case(title, minor_words = "")
#   title.split.map { |word| minor_words.downcase.include?(word.downcase) && word != title[0] ? word.downcase : word.capitalize }.join(" ")
# end
# # (title_case(''), '')
